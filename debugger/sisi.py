...
def get_net(batch_size=1, num_classes=10, image_shape=(1, 28, 28), dtype="float32"):
    ...
    B = relay.var("B", shape=data_shape, dtype=dtype)
    C = relay.nn.dot(A,B)
    sisi = relay.nn.bias_add(C, relay.var("bias"), axis=-1)
    """
    ...
    sisi = relay.nn.relu(fc1)

    args = relay.analysis.free_vars(sisi)
    return relay.Function(args, sisi)


def get_workload(batch_size=1, num_classes=10, image_shape=(1, 28, 28), dtype="float32"):
    ...
    -------
    mod : tvm.IRModule
        The relay module that contains a sisi network.
