...
def test_tune_matmul_cpu():
    ...
    target = Target("llvm --num-cores=4")
    work_dir = "./log"
...
def test_tune_matmul_cuda():
    # with tempfile.TemporaryDirectory() as work_dir:
    work_dir = "./log" 
    target = Target("nvidia/geforce-gtx-960")
    # cuda -keys=cuda,gpu -arch=sm_52 -max_num_threads=1024 -max_shared_memory_per_block=49152 -max_threads_per_block=1024 -registers_per_block=65536 -thread_warp_size=32
    database = ms.tir_integration.tune_tir(
        mod=matmul,
        target=target,
        work_dir=work_dir,
