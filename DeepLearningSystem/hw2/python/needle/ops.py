...
class MatMul(TensorOp):
    ...
    def gradient(self, out_grad, node):
        ...
        #     axes[-2] = last
        #     axes = list(axes)
        # rhs_t = Transpose(axes)(rhs)faxis
        ...
        # if out_len > lhs_len:
        #     lhs_grad = summation(lhs_grad, axes=tuple(range(out_len - lhs_len)))
        # if out_len > rhs_len:
        #     lhs_grad = summation(rhs_grad, axes=tuple(range(out_len - rhs_len)))
        lhs_grad_len = len(lhs_grad.shape)
        rhs_grad_len = len(rhs_grad.shape)
        if lhs_grad_len > lhs_len:
            for i in range(lhs_grad_len - lhs_len):
