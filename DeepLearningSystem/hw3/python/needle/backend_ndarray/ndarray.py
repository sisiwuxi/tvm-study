...
class NDArray:
    ...
    def permute(self, new_axes):
        ...
        #         )
        # return ret
        new_shape = [self._shape[i] for i in new_axes]
        new_strides = [self._strides[i] for i in new_axes]
        return NDArray.make(tuple(new_shape), tuple(new_strides), self.device, self._handle, self._offset)
        ### END YOUR SOLUTION
    ...
    def __getitem__(self, idxs):
        ...
        for i, s in enumerate(idxs):
            ...
            new_shape[i] = math.ceil((s.stop - s.start) / s.step)
            new_strides[i] = self.strides[i] * s.step
