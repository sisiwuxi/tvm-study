...
__global__ void reduceNeighbored(int * g_idata,int * g_odata,unsigned int n) 
{
    ...
    // // partialSum[tid] = GetElement(idata[tid],1024);
    // for (int i = 0; i < 1024; ++i)
    // {
    //     partialSum[i] = g_idata[i];
    // }
    // __syncthreads();
	...
	for (int stride = 1; stride < blockDim.x; stride *= 2)
	{
        // 1 cycle: thread 0,2,4,6,...
        // 2 cycle: thread 0,4,8,...
        // 3 cycle: thread 0,8,16,...
        // ...
		if ((tid % (2 * stride)) == 0)
		{
			idata[tid] += idata[tid + stride];
...
__global__ void reduceNeighboredLess(int * g_idata,int *g_odata,unsigned int n)
{
	...
	for (int stride = 1; stride < blockDim.x; stride *= 2)
	{
        ...
        // 1 cycle: thread 0,1,2,...,blockDim.x/2
        // 2 cycle: thread 0,1,2,...,blockDim.x/4
        // 3 cycle: thread 0,1,2,...,blockDim.x/8
        // ...        
		if (index < blockDim.x)
		{
